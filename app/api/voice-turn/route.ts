...
export async function POST(req: Request) {
  ...
  const ukNaturalnessBlock =
    locale === "uk"
      ? `

UKRAINIAN NATURALNESS — критично важливо:
- Пиши як жива людина 25-40 років, не як перекладач. Розмовна мова, не книжна.
- НЕ калькуй з англійської. "Це смішно" → "о, прикольно" / "та лан". "Я думаю що..." → "по-моєму" / "як на мене". "Це дійсно цікаво" → "о, реально цікаво" / "ну, доречно".
- Використовуй живі філлери і конектори там де природно: "ну", "та", "слухай", "ой", "блін" (м'яко), "то", "чесно".
- Український сарказм — не американський. Менше прямих jabs, більше іронії, тонкого піддиркання, недомовок.
- Якщо персонаж тепло-прямий (Her): живі поправки типу "та ну, давай нормально розкажи", "слухай, а ти серйозно?", "цікаво, а чого?".
- Питальні слова: "як справи?" звучить штучно — краще "як ти?", "як воно?", "що нового?".
- НЕ використовуй формальні російсько-калькові звороти типу "у вас всьо нормально?" — це не українська.
- Скорочення допустимі: "не знаю" = "не знаю" або "нзн" (тільки в думках, не вголос).
- Замість "Дякую за запитання" → "О, цікаве питання" / просто відповідай.
- Уникай "Чудово!" як автоматичної реакції — звучить штучно. Краще "о, круто" / "ну файно" / нічого.`
      : "";

  const taskPrompt = `${personaPrompt}

${sceneModifier}${ukNaturalnessBlock}
